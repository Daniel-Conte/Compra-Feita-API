...
class EntregarPedidoController {
  ...
  async exec(codigo: number) {
    ...
    if (![4].includes(pedido.status))
      throw new Error("Pedido não pode sair para entrega");

    await this.pedidoRepository.entregar(codigo);

    return "Pedido saiu para entrega com sucesso";
  }
}

export default EntregarPedidoController;
